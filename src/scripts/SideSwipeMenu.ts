...
export const useSwipeMenu = () => {
  ...
  onMounted(() => {
    ...
    window.addEventListener('touchmove', (e) =>
      touchXY.value = [e.changedTouches[0].pageX, e.changedTouches[0].pageY]
    )

    window.addEventListener('touchend', () => {
      if (Math.abs(touchXY.value[0] - startXY[0]) > Math.abs(touchXY.value[1] - startXY[1])) {
        if (touchXY.value[0] - startXY[0] > 0) { document.getElementById("hoverPage")?.animate([frameA, frameB], options); }
        else { document.getElementById("hoverPage")?.animate([frameB, frameA], options); }
      }
    })
  })
