...
const createEmojiIndex = async (type: string[], host: string) => {
  ...
  if (type.indexOf("category") !== -1) {
    ...
    emojis.forEach(emoji => {
      if (!categorys.hasOwnProperty(emoji.category))
        categorys[emoji.category] = [];
...
export const readEmojiIndex = (
  ...
  let localEmojis = localStorage.getItem(
    `${host}_emojis${type ? `_${type}` : ""}`
  );

  if (!localEmojis) {
    createEmojiIndex([type ?? ""], host);
    localEmojis = localStorage.getItem(
      `${host}_emojis${type ? `_${type}` : ""}`
    );
  }
